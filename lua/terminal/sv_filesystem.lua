...
Terminal.file = Terminal.file or {};

function Terminal.file:Initialize(entity)
	entity.fileCurrentDir = {};
end;

function Terminal.file:ChangeDir(entity, key)
	...
	Terminal:Broadcast(entity, "Unable to find directory!");

	return false;
end;

function Terminal.file:Write(entity, key, value)
	if (!entity.fileCurrentDir) then
		entity.fileCurrentDir = {};
	end;
	
	if (!key or key == "_parent") then
		Terminal:Broadcast(entity, "Invalid name!");

		return false;
	end;

	if ( entity.fileCurrentDir[key] ) then
		Terminal:Broadcast(entity, "Item already exists!");

		return false;
	end;		

	if (!value) then
		Terminal:Broadcast(entity, "Invalid content!");
...
function Terminal.file:Rename(entity, previous, new)
	if ( !previous or !entity.fileCurrentDir[previous] ) then
		Terminal:Broadcast(entity, "Unable to find file/directory!");

		return false;
	end;

	if (!new or new  == "_parent") then
		Terminal:Broadcast(entity, "Invalid file/directory name!");
...
function Terminal.file:Delete(entity, key)
	if ( !key or key == "_parent" or !entity.fileCurrentDir[key] ) then
		Terminal:Broadcast(entity, "Invalid file/directory!");
...
function Terminal.file:Read(entity, key)
	if (!key or key == "_parent") then
		Terminal:Broadcast(entity, "Invalid name!");
	...
	if ( !entity.fileCurrentDir[key] ) then
		Terminal:Broadcast(entity, "Couldn't find file!");

		return false;
	end;

	if (!entity.fileCurrentDir[key].isFile or !entity.fileCurrentDir[key].value) then
		Terminal:Broadcast(entity, "Invalid read type!");
