...
ENT.Category = "Non Player Characters"
ENT.Author = "Cartmanium, Chessnut"
ENT.Spawnable = true
ENT.AdminOnly = true
...
function ENT:AlertNearby(target, range, noNoise)
        range = range or 2400
        noNoise = noNoise or (#ents.FindByClass("npc_thezombie") < 1)

        if (IsValid(self.target)) then
                return
        end

        for k, v in pairs(ents.FindByClass("npc_thezombie")) do
