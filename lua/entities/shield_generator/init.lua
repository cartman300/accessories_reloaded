...
function ENT:Initialize()
	self.AntiNoclip = true
	self.AtlantisMode = false
...
function ENT:TakeEnergy(shot, dmge)
	if shot then
		if dmge != nil then
			self:ConsumeResource("energy",(500*dmge^2)/4);
		else
			self:ConsumeResource("energy",5000/2);
		end
	else
		self:ConsumeResource("energy",5000/2);
	end
...
function ENT:Think()
	...
	elseif(enabled and self.HasResourceDistribution and self.ConsumeMultiplier ~= 0) then
		if self.AtlantisMode then
			...
			local damage_energy = take_energy*2
			if energy >= 5000 then
				self:TakeEnergy(false)
			else
				self:Status(false);
				return;
			end
...
function ENT:Hit(strength,normal,pos)
	...
	if self.AtlantisMode then
		self.Strength = 100
		self:TakeEnergy(true, math.Clamp(strength,1,100))
	else
