...
ENT.PrintName		= "Naquadah Missile"
...
if SERVER then
	...
	function ENT:SpawnFunction( ply, tr )
		...
		local ent = ents.Create( "agate_naq_missle" )
		ent:SetPos( SpawnPos )
		ent:Spawn()
		ent:Activate()
	...
	function ENT:Detonate()
	   ...
	   if(warhead ~= nil and warhead:IsValid()) then
		  warhead:Setup(self.Entity:GetPos(), 7)
	...
	function ENT:PreEntityCopy()
		...
		duplicator.StoreEntityModifier(self, "AGateMissileN", dupeInfo)
	end
	duplicator.RegisterEntityModifier( "AGateMissileN" , function() end)

	function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
		
		local dupeInfo = Ent.EntityMods.AGateMissileN

		if dupeInfo.EntID then
			self.Entity = CreatedEntities[ dupeInfo.EntID ]
		end

		if(Ent.EntityMods and Ent.EntityMods.AGateMissileN.WireData) then
			WireLib.ApplyDupeInfo( ply, Ent, Ent.EntityMods.AGateMissileN.WireData, function(id) return CreatedEntities[id] end)
		end
