...
local function OnSndNodeSelected( self, node, name, path, pathid, icon, ViewPanel, pnlContent )
	...
	for k, v in pairs( files ) do
		...
		local p = Path .. "/"
		if ( string.StartWith( path, "addons/" ) ) then
...
hook.Add( "PopulateContent", "SpawnmenuLoadSomeSounds", function( pnlContent, tree, browseNode ) timer.Simple( 0.5, function()
	...
	local browseGameSounds = browseSounds:AddNode( "Games", "icon16/folder_database.png" )

	AddBrowseContentSnd( ViewPanel, browseGameSounds, "All", "games/16/all.png", "", "GAME", pnlContent )
	AddBrowseContentSnd( ViewPanel, browseGameSounds, "Garry's Mod", "games/16/garrysmod.png", "", "garrysmod", pnlContent )

	local games = engine.GetGames()
	for _, game in SortedPairs( games ) do
		if ( !game.mounted ) then continue end
		AddBrowseContentSnd( ViewPanel, browseGameSounds, game.folder, "games/16/" .. game.folder .. ".png", "", game.folder, pnlContent )
	end
			
	/* --------------------------------------------------------------------------------------- */
			
	local browseAddonSounds = browseSounds:AddNode( "Addons", "icon16/folder_database.png" )
	...
	local browseLegacySounds = browseSounds:AddNode( "Legacy Addons", "icon16/folder_database.png" )
...
spawnmenu.AddContentType( "material", function( container, obj )
	...
	icon.OpenMenu = function( icon ) 
		local menu = DermaMenu()
			menu:AddOption( "Copy path to Clipboard", function() SetClipboardText( obj.spawnname ) end )
			if ( string.GetExtensionFromFilename( obj.spawnname ) == "vmt" ) then
				menu:AddOption( "Set as material for Material tool", function() RunConsoleCommand( "material_override", obj.spawnname ) end )
			end
			menu:AddSpacer()							
			menu:AddOption( "Delete", function() icon:Remove() hook.Run( "SpawnlistContentChanged", icon ) end )
		menu:Open()
...
hook.Add( "PopulateContent", "SpawnmenuLoadSomeMaterials", function( pnlContent, tree, browseNode ) timer.Simple( 0.5, function()
	...
	local browseGameMaterials = browseMaterials:AddNode( "Games", "icon16/folder_database.png" )
			
	AddBrowseContentMaterial( ViewPanel, browseGameMaterials, "All", "games/16/all.png", "", "GAME", pnlContent )
	AddBrowseContentMaterial( ViewPanel, browseGameMaterials, "Garry's Mod", "games/16/garrysmod.png", "", "garrysmod", pnlContent )

	local games = engine.GetGames()
	for _, game in SortedPairs( games ) do
		if ( !game.mounted ) then continue end
		AddBrowseContentMaterial( ViewPanel, browseGameMaterials, game.folder, "games/16/" .. game.folder .. ".png", "", game.folder, pnlContent )
	end

	/* --------------------------------------------------------------------------------------- */

	local browseAddonMaterials = browseMaterials:AddNode( "Addons", "icon16/folder_database.png" )
	...
	local browseLegacyMaterials = browseMaterials:AddNode( "Legacy Addons", "icon16/folder_database.png" )
...
hook.Add( "PopulatePropMenu", "LoadLegacySpawnlists", function()

	local sid = table.Count( spawnmenu.GetPropTable() )

	local added = false

	for id, spawnlist in pairs( file.Find( "settings/spawnlist/*.txt", "GAME" ) ) do
		local content = file.Read( "settings/spawnlist/" .. spawnlist, "GAME" )
		if ( !content ) then continue end
					
		local is = content:sub( 1, content:find( "\n" ) - 1 ):find( "TableToKeyValues" )
		if ( is != nil ) then continue end
		
		for id, t in pairs( spawnmenu.GetPropTable() ) do
